...
/**
 * Helpless copy the code, because they can not inherit XlsTable, and XlsTable can not read from the
 * PL/Sql developer to export excel data Major rewrite of the (@ link # getValue (int, String))
 * method. Also increases the counting method of handling scientific.
 * 
 * @version ScsDI v1.0
 * @author Zhang Ran, 2018年4月20日
 */
public class XlsTable extends AbstractTable {
    private static final Pattern pattern = Pattern.compile("[eE]");

    /**
     * Logger for this class.
     */
    private static final Logger log = LoggerFactory.getLogger(XlsTable.class);
    ...
    public XlsTable(HSSFWorkbook workbook, String sheetName, HSSFSheet sheet)
    ...
    static ITableMetaData createMetaData(String tableName, HSSFRow sampleRow, HSSFWorkbook workbook) {
        log.debug("createMetaData(tableName={}, sampleRow={}) - start", tableName, sampleRow);
    ...
    protected BigDecimal getNumericValue(HSSFCell cell) {
        ...
        if (resultString != null) {
            try {
                ...
                log.debug("Exception occurred while trying create a BigDecimal. value={}",
                        resultString);
    ...
    public Object getValue(int row, String column) throws DataSetException {
        if (log.isDebugEnabled())
            log.debug("getValue(row={}, columnName={}) - start", Integer.toString(row), column);
        ...
        switch (type) {
            case HSSFCell.CELL_TYPE_NUMERIC:
                ...
                } else if (XlsDataSetWriter.DATE_FORMAT_AS_NUMBER_DBUNIT.equals(style
                        .getDataFormatString())) {
            ...
            case HSSFCell.CELL_TYPE_FORMULA:
                throw new DataTypeException("Formula not supported at row=" + row + ", column="
                        + column);
