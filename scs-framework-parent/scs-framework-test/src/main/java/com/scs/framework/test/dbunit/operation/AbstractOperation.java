...
public abstract class AbstractOperation extends DatabaseOperation {
    ...
    /**
     * Returns the metadata to use in this operation. It is retrieved from the database connection
     * using the information from the physical database table.
     * 
    ...
    static ITableMetaData getOperationMetaData(IDatabaseConnection connection,
        ...
        logger.debug("getOperationMetaData(connection={}, metaData={}) - start", connection,
                metaData);
        ...
        for (int j = 0; j < columns.length; j++) {
            ...
            // method "getColumnIndex()" throws NoSuchColumnsException when columns have not been
            // found
            ...
            if (!ArrayUtils.isEmpty(primaryKeys) && ArrayUtils.contains(primaryKeys, columns[j])) {
        ...
        if (primarykeyList.size() > 0) {
            primaryKeys = primarykeyList.toArray(new Column[primarykeyList.size()]);
        }
